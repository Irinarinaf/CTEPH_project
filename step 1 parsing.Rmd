...
# Function for filling in mutation on all visits
fixed_values <- function(ids) {
...
# calculation of the last visit before the start of treatment
...
data_surv <- data%>%
  ...
  mutate(Age = as.numeric(difftime(OS1_startdt, btdt, units = "days"))/ 30.4375 / 12,   # age at the start of treatment
...
data_surv_new <- data_surv%>%
  ...
  select(!c(ECHOCGmeanDLA, ECHOCGApicalSize, RHCathetermeanDLA, RHCatheterDDLA, RHCatheterCV, RHCatheterOLCC, pro_BNP, WeightEnd,BPAdate, DateTPE, btdt))%>%
